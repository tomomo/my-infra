# ==========================================
# AlmaLinux 9 - PHP 8.1 & Node.js 20 Environment
...
# 4. PHP 8.1 / Node.js 20 / 開発ツールのインストール
# COPY --from を使わず、ビルド環境のアーキテクチャに追従させる
RUN microdnf module enable php:remi-8.1 nodejs:20 -y \
    && microdnf install -y --refresh \
       php-{cli,fpm,intl,mbstring,xml,zip,gd,mysqlnd,opcache,imap,pgsql,redis,devel,pecl-imagick-im6} \
       nodejs npm \
       tzdata git git-lfs unzip procps-ng vim-minimal findutils shadow-utils libstdc++ \
       hostname iproute iputils which \
...
RUN { \
    ...
    echo 'export PS1="\[\e[1;32m\]\u@\h\[\e[m\]:\[\e[1;36m\]\W\[\e[1;33m\]\$(__git_ps1 \" (%s)\")\[\e[1;32m\]\\$ \[\e[m\]"'; \
    echo "alias ls='ls --color=auto'"; \
    echo "alias ll='ls -l'"; \
    } > /etc/profile.d/terminal_setup.sh \
    && echo "source /etc/profile.d/terminal_setup.sh" >> /etc/bashrc
